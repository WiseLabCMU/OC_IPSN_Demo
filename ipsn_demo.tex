...
%\title{Demo Abstract: Let's start Chirping with OpenChirp}

% author names and affiliations
% use a multiple column layout for up to three different
% affiliations
\author{\IEEEauthorblockN{Adwait Dongare, Anh Luong, Khushboo Bhatia, Craig Hesling, Anthony Rowe}
...
Smart home devices are ubiquitous in today's home. However, current indoor communication technologies while great indoor, has limited ranges and power hungry.

\section{The OpenChirp Architecture}

[Add architecture picture]

\section{LPRAN: A low-cost SDR for the LPWAN world}
...
% Can we bring a bunch of LoRaBug + Sensor Board
% set them up at the door
% and do headcounting?

In this demonstration, we want to show the simplicity of creating a network based on the OC framework. Throughout the conference halls, we have deployed LoRa devices connected to the gateway we have on the demo table. These includes COTS LoPy with PySense and PyTrack expansion boards, and custom LoRaBug with sensor boards. With the array of sensors, we show that with the current infrastructure, we are able to support a large scale sensing network. On the OC website, we show the simplicity of the data visualization and raw data collection through OC website.
